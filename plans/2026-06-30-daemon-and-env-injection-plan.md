...
- **2f — macOS peer-cred parity (owner-scoped 2026-07-01); Windows stays on fallback.** macOS `authorizePeer` via `getsockopt(LOCAL_PEERCRED)` → `Xucred.Uid`, sharing the `authorizedUID` decision with Linux (fail-closed). `authorizedUID` moved to `peercred_unix.go` (`linux || darwin`); the no-op stub retags to `peercred_other.go` (`!linux && !darwin`); the peer-cred tests run on `linux || darwin` so macOS CI exercises `GetsockoptXucred`. **Windows named-pipe transport + ACL is explicitly NOT built** (owner decision: high effort / CI-only-testable / low ROI given the working direct-open fallback) — Windows keeps `ErrUnsupported` + fallback. macOS memory hardening is left for later (weaker mlock story; also depends on 2d landing). Security-critical. **Hold for review.**
